...
pokemon_dataset = load_dataset("lca0503")

# Load the colormap from Hugging Face Hub
colormap = list(load_dataset("lca0503colormap")["train"]["color"])
...
qwen2_config = Qwen2Config(
    vocab_size=num_classes,       # Number of classes (equivalent to vocabulary size)
    hidden_size=128,              # Embedding dimension (recommended to increase to 128 for better stability)
    num_hidden_layers=3,          # transformer layers (slightly stronger than 2 layers)
    num_attention_heads=4,        # attention heads
    num_key_value_heads=4,        # Number of key/value heads, aligned with num_attention_heads
    intermediate_size=512,        # FFN layer size (typically hidden_size * 4)
    max_position_embeddings=400,  # Sequence length (20x20)
    pad_token_id=None,
    eos_token_id=None,
    hidden_act="gelu",
    rms_norm_eps=1e-06
...
class PokemonClassifier(nn.Module):
    def __init__(self):
        ...
        self.fc1 = nn.Linear(32 * 5 * 5, 64)   # 因为 20x20 -> 10x10 -> 5x5
        self.fc2 = nn.Linear(64, 2)            # 输出2类 (0=not Pokémon, 1=Pokémon)

    def forward(self, x):
        x = self.pool(F.relu(self.conv1(x)))  # [B,16,10,10]
...
# 正样本 (Pokémon)
positive_samples = [image_to_tensor(img) for img in train_images]
positive_labels = [1] * len(positive_samples)

# 负样本 (噪声图像)
negative_samples = [torch.rand(3,20,20) for _ in range(len(positive_samples))]
negative_labels = [0] * len(negative_samples)

# 合并数据
X = torch.stack(positive_samples + negative_samples)
y = torch.tensor(positive_labels + negative_labels)

classifier_dataset = torch.utils.data.TensorDataset(X, y)
classifier_loader = DataLoader(classifier_dataset, batch_size=32, shuffle=True)
...
for epoch in range(5):  # 训练 5 轮就够
    ...
    print(f"[Classifier] Epoch {epoch+1}, Loss: {total_loss/len(classifier_loader):.4f}")

# 把生成的 predicted_images 转成张量
gen_tensors = torch.stack([image_to_tensor(img) for img in predicted_images]).to(device)

classifier.eval()
with torch.no_grad():
    outputs = classifier(gen_tensors)
    preds = torch.argmax(outputs, dim=1)
    PDR = preds.sum().item() / len(preds)

print(f"Pokémon Detection Rate (PDR): {PDR:.4f}")
